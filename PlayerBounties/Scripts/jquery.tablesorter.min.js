(function($)
{
	$.extend({ tablesorter: new function()
	{
		...
		function computeTableHeaderCellIndexes(t)
		{
			...
			for(var i = 0; i < trs.length; i++)
			{
				...
				for(var j = 0; j < cells.length; j++)
				{
					...
					{
						if(typeof (matrix[rowIndex][k]) == "undefined") { firstAvailCol = k; break; }
					}
		...
		function checkHeaderOptionsSortingLocked(table, i)
		{
			...
			function setHeadersCss(table, $headers, list, css)
			{
				...
				var h = []; $headers.each(function(offset)
				{
					if(!this.sortDisabled)
					{
						h[this.column] = $(this);
			...
			function multisort(table, sortList, cache)
			{
				...
				for(var i = 0; i < l; i++)
				{
					...
					var e = "e" + i; dynamicExp += "var " + e + " = " + s; dynamicExp += "if(" + e + ") { return " + e + "; } ";
					dynamicExp += "else { ";
				}

				var orgOrderCol = cache.normalized[0].length - 1; dynamicExp += "return a[" + orgOrderCol + "]-b[" + orgOrderCol + "];";
						...
						if(!this.sortDisabled && totalRows > 0)
						{
							...
							if(!e[config.sortMultiSortKey])
							{
								...
								if(config.sortForce != null)
								{
									...
									{
										if(a[j][0] != i) { config.sortList.push(a[j]); } 
									} 
								}
...

			{
				if($.browser.msie)
				{
					function empty() { while(this.firstChild) this.removeChild(this.firstChild); } empty.apply(table.tBodies[0]); 
				} else
				{
	...
	ts.addWidget({ 
		...
		format: function(table)
		{
			...
			var $tr, row = -1,
			odd; $("tr:visible",
			table.tBodies[0]).each(function(i)
			{
				$tr = $(this);
